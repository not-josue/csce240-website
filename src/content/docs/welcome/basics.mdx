...
editor you're comfortable with. For this course, you will be saving saving files with the following extensions: `.cc`, `.h`, and `makefile`.
...
These directives are executed to prepare the code for compilation. Common tasks performed during this step include including header files 
...
This terminal command creates a `driver` executable file. Most if not all example code in this course uses the default `a.out`.
