...
is **C_Cpp: Clang_format_style** and its value should be `file`.
    ...
    <Aside>
    What we did was change the default file formatter to `clang` and require it to check for a `clang-format file` that defines its styles. 
    Since we won't be using a `clang-format file`, then it will use the fallback styling preferences (which we changed to be `Google`).
    </Aside>
